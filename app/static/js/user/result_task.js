$(document).ready(function () {
    ...
    $(str).each(function (indx, item) {
        ...
        let indxBegin = item.indexOf("<"),
            indxEnd =
                ...
                item.indexOf(" ", indxBegin) == -1
                    ? item.indexOf(">", indxBegin)
                    : item.indexOf(" ", indxBegin),
            classStr = item.substr(indxBegin + 1, indxEnd - indxBegin - 1);
        // сведение классов типа "class" и "/class" к одному типу "class"
        if (classStr.indexOf("/") != -1)
            classStr = classStr.substr(
                classStr.indexOf("/") + 1,
                classStr.length - 1
            );
        let elemText = item.substr(
                item.indexOf(classStr) + classStr.length,
                item.length
            ),
            openTeg = "<" + classStr,
            closeTeg;
        ...
        if (classStr.length > 0) {
            elem.append(
                $(
                    "<span class='teg' style='padding-left: " +
                        (12 + indxBegin * 6) +
                        "px'></span>"
                ).text(openTeg)
            );
